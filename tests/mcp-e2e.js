...
(async () => {
  try {
    ...
    expect(names.length === 20 && names.includes('create_map') && names.includes('create_rule') && names.includes('get_org_structure') && names.includes('list_people') && names.includes('get_portfolio'),
      `tools/list → 20 nástrojů vč. list_people a get_portfolio (${names.join(', ')})`);
