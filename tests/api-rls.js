// RLS / autorizační smoke testy — čerstvý kontejner na :20490, jen API (bez prohlížeče).
...
const BASE = 'http://127.0.0.1:20490';
const NAME = 'flowmap-e2e-rls';
...
(async () => {
  try {
    ...
    execSync(`docker run -d --name ${NAME} -e KB_UVODNI_MAPA=0 -p 20490:8090 ${process.env.KB_TEST_IMAGE || 'product-flowmap'}`, { stdio: 'ignore' });
